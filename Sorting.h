...
int bubble_sort(int arr[], int size) {
  ...
  do {
    ...
    for(int count = 0; count < (size - 1); count++) {
      if((comps++,(arr[count] > arr[count + 1]))) {
        temp = arr[count];
...
int insertion_sort (int arr[], int size) {
  ...
  for(int i = 1, j; i < size; i++) {
    int temp = arr[i];
    for(j = i; j > 0 && (count++,(temp < arr[j - 1])); j--) {
      arr[j] = arr[j - 1];
    }
    arr[j] = temp;
  }
 ...
 */
int partition(int arr[], int left_index, int right_index, bool is_random_pivot, int& count) {
  ...
  for(int i = left_index; i <= right_index - 1; i++){       // each i from left_index to right_index-1: // by scanning the entire arr
    if((count++,(arr[i] < pivot_value))){                               // if value smaller than pivot is found
 ...
 * @param right_index             The right index of an array.
 */
void quick_sort(int arr[], int left_index, int right_index, int& count) {
  if(left_index < right_index){                                    // being "unsorted" requires 2 or more values...
    int division = partition(arr, left_index, right_index, false, count);        // smaller values to left of division, larger to right
    quick_sort(arr, left_index, division-1, count);                        // now quicksort everything left of division
    quick_sort(arr, division+1, right_index, count);                       // and everything right of division
  }
}
 ...
 * @param right_index             The right index of an array.
 */
void random_quick_sort(int arr[], int left_index, int right_index, int& count) {
  if(left_index < right_index){                                    // being "unsorted" requires 2 or more values...
    int division = partition(arr, left_index, right_index, true, count);        // smaller values to left of division, larger to right
    random_quick_sort(arr, left_index, division-1, count);                        // now quicksort everything left of division
    random_quick_sort(arr, division+1, right_index, count);                       // and everything right of division
  }
  return;
}
...
int random_quick_sort(int arr[], int size) {
  int count = 0;
  random_quick_sort(arr, 0, size - 1, count);
  return count;
}
 ...
 * @param right_index             The right index of an array.
 */
void max_heap_sift_down(int heap_array[], int left_index, int right_index, int& count){
  ...
  while(!done && left(i) <= right_index){                                            // scanning until done or h[i] has no children
      ...
      if(right(i) <= right_index && (count++,(heap_array[right(i)] > heap_array[left(i)]))){     // see if right child exists and is larger than left
          max_child = right(i);                                                      // if so, mark right child as max_child
      }
      if((count++,(heap_array[max_child] > heap_array[i]))){                                     // if the max_child is bigger than h[i]
 ...
 */
void max_heap_sift_down(int heap_array[], int heap_size,int& count){
  max_heap_sift_down(heap_array, 0, heap_size-1, count);     // calls the more general algorithm shown below
}
 ...
 */
int heap_remove(int heap_array[], int & heap_size, int& count) {
  ...
  max_heap_sift_down(heap_array, heap_size, count);          // sift down the element just swapped to root
 ...
 * @param size             The size of the array.
 */
void make_max_heap_bottom_up(int arr[], int size, int& count){
  int subroot = parent(size - 1);                     // start with the last non-leaf node
  while(subroot >= 0){                                // run until we reach the true root
    max_heap_sift_down(arr, subroot, size-1, count);         // sift down the new subroot's value
    subroot--;
  }
}
...
int heap_sort(int arr[], int size){
  int count = 0;
  make_max_heap_bottom_up(arr, size, count);                 // begin by making the array a heap
  ...
  while(elements_remaining > 1){                      // while the unsorted part of the array is non-empty
    heap_remove(arr, elements_remaining, count);             // "remove" the max (which moves it to the end!)
  }
  return count;
}
